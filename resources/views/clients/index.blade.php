...
@section('additional_scripts')
    ...
    <script>
        ...
        function getCities(){
          ...
          console.log(govern);
          $.ajax({
            url:`https://blood-bank.devhegz/api/v1/cities?govern=${govern}`
          }).done(function(data){
...
@section('content')
    ...
    <section class="content">
      ...
      <div class="input-group input-group-sm mr-2">
        <input class="form-control form-control-navbar" type="search" name="search" value="" style="background-color: #fff;" placeholder="Search" aria-label="Search">
        <div class="input-group-append">
          <button class="btn btn-navbar" style="background-color: #fff; border:1px solid #CED4DA; border-left:0; color:rgba(0,0,0,.6)" type="submit">
            ...
            class="table table-bordered table-hover table-striped">
                ...
                <tbody>
                   @forelse ($records as $record)
                       ...
                       <td>
                       <a href="{{route('client.update',['client'=>$record->id])}}" 
                        onclick="event.preventDefault();
                                    document.getElementById('{{'ban'.$record->id}}').submit();"
                        class="btn btn-warning"><i class="fas fa-user-slash"></i></a>
                       <form id="{{'ban'.$record->id}}" action="{{ route('client.update',['client'=>$record->id]) }}" method="POST" style="display: none;">
                                            @method('put')
                       ...
                       <td>
                           <a href="{{route('client.destroy',['client'=>$record->id])}}"
                            ...
                            class="btn btn-danger"><i class="fas fa-trash"></i></a>
                                        <form id="{{'delete'.$record->id}}" action="{{ route('client.destroy',['client'=>$record->id]) }}" method="POST" style="display: none;">
                                            @method('delete')
                                        @csrf
                                    </form>
                       </td>
